...
class WonderPlugin_Carousel_View {
	...
	function print_register() {
		...
		if (isset($_POST['save-carousel-license']))
		{		
		...
		else if (isset($_POST['deregister-carousel-license']))
		{	
		...
		else
		{
			...
			<?php } else { ?>
				...
				<tr>
					...
					<td><?php echo $key; ?> &nbsp;&nbsp;&nbsp;&nbsp;<input name="wonderplugin-carousel-key" type="hidden" id="wonderplugin-carousel-key" value="<?php echo $key; ?>" class="regular-text" /><input type="submit" name="deregister-carousel-license" id="deregister-carousel-license" class="button button-primary" value="Deregister the License Key"  /></label></td>
				</tr>
				</table>
				</form>
	...
	function print_add_new() {
		...
		if ( !empty($_POST['wonderplugin-carousel-save-item-post-value']) && !empty($_POST['wonderplugin-carousel-save-item-post']) )
		{
	...
	function save_item_post($item_post) {
		...
		if (isset($ret['success']) && $ret['success'] && isset($ret['id']) && $ret['id'] >= 0) 
		{
			...
			echo "<div class='updated'><p>To embed the carousel into your page or post, use shortcode <b>[wonderplugin_carousel id=\"" . $ret['id'] . "\"]</b></p></div>";
			echo "<div class='updated'><p>To embed the carousel into your template, use php code <b>&lt;?php echo do_shortcode('[wonderplugin_carousel id=\"" . $ret['id'] . "\"]'); ?&gt;</b></p></div>"; 
		}
		else
		{
